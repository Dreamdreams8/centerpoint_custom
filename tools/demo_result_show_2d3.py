...
def visualize_result(points, detection, save_path, point_cloud_range, gt_detection=None):
    """可视化点云和检测结果并保存"""
    ...
    if gt_detection is not None:
        gt_boxes = gt_detection['box3d_lidar']
        gt_labels = gt_detection['label_preds']
        for i in range(len(gt_boxes)):
            color = box_colormap[gt_labels[i]]
            # 使用虚线绘制GT框
            draw_2d_box(ax, gt_boxes[i,0], gt_boxes[i,1], 
                       gt_boxes[i,4], gt_boxes[i,3], -gt_boxes[i,8], 
                       color=(*color, 0.5))  # 半透明效果
...
def main():
    ...
    # 加载checkpoint
    checkpoint = load_checkpoint(model, 'checkpoint/epoch_61.pth', map_location="cpu")
    model.eval()
    model = model.cuda()
    ...
    # 创建输出目录
    out_dir = "data/custom_zzg_0325_2class_11846_1500/visualization"
    ...
    for i, data_batch in tqdm(enumerate(data_loader), total=len(data_loader)):
        ...
        # 获取GT数据
        gt_info = data_batch['metadata'][0]
        print("gt_info: ",gt_info)
        gt_detection = convert_box(gt_info)
        ...
        # 保存GT结果可视化
        gt_save_path = os.path.join(gt_dir, f"{lidar_token}.png")
        visualize_result(points.T, gt_detection, gt_save_path, point_cloud_range)
